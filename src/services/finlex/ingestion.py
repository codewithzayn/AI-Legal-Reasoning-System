...
class FinlexIngestionService:
    ...
    async def process_document(
        ...
        """
        try:
            # 0. Check Existence (if not forced)
            if not force_reingest:
                exists = self.storage.client.table('legal_chunks').select('id').eq(
            ...
            # If reingesting, clear old chunks
            if force_reingest:
                 self.storage.client.table('legal_chunks').delete().eq(
                    'document_uri', document_uri
                ).execute()

            # 1. Fetch document (async)
            ...
            # 3. Handle embedded PDF links (e.g. for judgments) - PARALLEL DOWNLOAD
            ...
            try:
                self.storage.client.table('failed_documents').delete().eq(
                    'document_uri', document_uri
                ).execute()
            except:
                pass
    ...
    def _update_tracking(self, document_category, document_type, document_year):
        try:
            tracking_check = self.storage.client.table('ingestion_tracking').select('*').eq(
                'document_category', document_category
