...
async def ingest_documents(request: IngestRequest):
    ...
    for doc_item in documents_to_process:
        ...
        try:
            ...
            error_msg = str(e)
            results.append(
                {"document_uri": document_uri, "success": False, "message": f"Failed: {error_msg}", "chunks_stored": 0}
            )
            failed += 1
...
async def retry_failed_documents(request: RetryRequest):
    ...
    failed_docs_response = (
        service.storage.client.table("failed_documents").select("*").lt("retry_count", request.max_retries).execute()
    )

    if not failed_docs_response.data:
        return IngestResponse(
            success=True, message="No failed documents to retry", total_documents=0, successful=0, failed=0, results=[]
        )
    ...
    for doc in failed_docs_response.data:
        ...
        try:
            ...
            service.storage.client.table("failed_documents").update(
                {"retry_count": new_retry_count, "last_retry_at": "now()", "error_message": error_msg}
